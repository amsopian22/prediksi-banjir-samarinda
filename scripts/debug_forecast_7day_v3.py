...
def debug_7day_forecast():
    ...
    for date_val, group in future_groups[:7]:
        ...
        # Test with Low Cumsum
        d_input_low_cum = d_input.copy()
        d_input_low_cum["rain_cumsum_7d"] = 10.0 # Force low
        d_input_low_cum["rain_cumsum_3d"] = 5.0
        
        result = model_utils.predict_flood(model_pack, d_input)
        result_low = model_utils.predict_flood(model_pack, d_input_low_cum)

        print(f"{str(date_val):<15} | {daily_rain:<10.2f} | {rain_cumsum_7d:<10.2f} | {result['probability']*100:<8.1f} | {result_low['probability']*100:<8.1f} (LowCum)")
