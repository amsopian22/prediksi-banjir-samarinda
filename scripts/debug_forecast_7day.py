...
def debug_7day_forecast():
    ...
    for date_val, group in future_groups[:7]:
        ...
        # Try to get raw model prob without processing
        # This is strictly for debug, assuming predict_flood calls the model
        model = model_pack.get("model")
        raw_prob_v = 0
        if model:
            # We need to construct DataFrame as model expects
            # This is complex to replicate 1:1, so we rely on result['probability'] roughly
            # But the result['probability'] in predict_flood is after any heuristics (which we enabled/disabled)
            pass

        print(f"{str(date_val):<15} | {daily_rain:<10.2f} | {daily_max_tide:<10.2f} | {rain_cumsum_7d:<10.2f} | {result['probability']*100:<8.1f} | {result['label']:<10} | {result['probability']:.4f}")
