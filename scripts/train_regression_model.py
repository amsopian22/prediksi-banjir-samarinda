...
def train_model():
    ...
    # Ensure we only take numeric columns and exclude target/metadata
    df_numeric = df.select_dtypes(include=[np.number])
    exclude_cols = ['water_depth_cm', 'label', 'status', 'level']
    
    features = [c for c in df_numeric.columns if c not in exclude_cols]
    
    # Handle duplicate columns (keep first)
