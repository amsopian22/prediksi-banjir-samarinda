...
def run_verification():
    ...
    # Check if model is available before proceeding with model-dependent tests
    if not model_pack:
        print("\n⚠️ SKIPPING TESTS 3-6: Model is unavailable")
        print("   -> Ensure 'models/model_banjir_v8_10years.pkl' exists in the repository")
        total_tests += 4  # Count them as failed
        issues.append("Tests 3-6 skipped due to missing model")
    else:
